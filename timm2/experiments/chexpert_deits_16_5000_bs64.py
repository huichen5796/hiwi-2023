...
def parameters(): 
    defaults = {
        ...
        #'data-file-train': '../data-local/labels/chexpert/chexpert_5000_00.csv',
        'data-file-val': '../data-local/chexpert_frap_binary_valid.csv',
    ...
    # Model parameters
        'model': 'deit_small_patch16_224', #'resnetv2_50x3_bitm'
        'pretrained': 'true',
        ...
        'warmup-lr': 1e-6, #  0, #help='warmup learning rate (default: 0.0001)')
        'min-lr': 0, #1e-6, #help='lower lr bound for cyclic schedulers that hit 0 (1e-5)')
...
def run(dataset, model,  n_labels, data_seed, **kwargs):
    ...
    adapted_args = {
        'dataset': dataset,
        'model': model,
        'data-file-train': f'../data-local/labels/{dataset}/{dataset}_{n_labels}_{data_seed:02d}.csv',
