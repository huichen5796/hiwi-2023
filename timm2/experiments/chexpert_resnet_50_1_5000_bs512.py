...
def parameters(): 
    defaults = {
        ...
        'epochs': 300,  # help='number of epochs to train (default: 300)')
        'epoch-repeats': 0.,
        # help='epoch repeat multiplier (number of times to repeat dataset epoch per train epoch).')
        'start-epoch': 0,  # help='manual epoch number (useful on restarts)')

    # Model parameters
        'model': 'resnet50', #'resnetv2_50x3_bitm'
        'pretrained': 'true',
        ...
        'warmup-lr': 1e-6,#0.0001, #help='warmup learning rate (default: 0.0001)')
        'min-lr': 0,#1e-6, #help='lower lr bound for cyclic schedulers that hit 0 (1e-5)')
        'decay-epochs': [30, 60, 90], #help='epoch interval to decay LR')
