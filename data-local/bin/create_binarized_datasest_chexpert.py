...
datafile = '/home/medssl/data/CheXpert-v1.0-small/valid.csv'
outdir = 'chexpert'
...
def reduced_binary_datasets(reduce=True, split='train'):
    ...
    df4 = df.filter(items=['Path', 'Sex', 'Age', 'Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Pleural Effusion'])
    ...
    if reduce:
        filename = f'../chexpert_frap_binary_{split}_official.csv'
    else:
        filename = f'../chexpert_binary_{split}.csv'
...
def limited_datasets():
    ...
    for seed in range(10):
        ...
        filename = f'../labels/{outdir}/{dataset_name}_{num_labels}_{seed:02d}.csv'
        print(filename)
...
if __name__ == '__main__':
    # reduced_binary_datasets(reduce=False, split='train')
    # reduced_binary_datasets(reduce=False, split='valid')
    #reduced_binary_datasets(reduce=False, split='valid')
    reduced_binary_datasets(reduce=True, split='valid')
    # pass
